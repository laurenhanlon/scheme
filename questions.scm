...
(define (zip pairs)
  (cons (apply-to-all car pairs)
   (cons (apply-to-all cadr pairs) nil)))
...
(define (list-partitions total)
  (define (partitions a b i lst)
    (cond ((= a 0) (list lst))
          ((< a 0) nil)
          ((= b 0) nil)
          ((< b 0) nil)
          (i (append (partitions (- a b) b #t
              (append lst (list b)))
          (partitions a (- b 2) #f lst)))
          (else (append (partitions (- a b) b #t
            (append lst (list b))) 
          (partitions a (- b 1) #f lst)))))
    (partitions total total #f nil))
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
            (cons (cons 'lambda (cons (car (zip values)) body)) 
            (cdr (zip values)))
           ))
        (else
