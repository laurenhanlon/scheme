...
def eval_all(expressions, env):
    ...
    if len(expressions)==0:
        return okay 
    return [scheme_eval(expression, env) for expression in expressions][-1]
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("formal parameters does not match arguements")
...
def do_and_form(expressions, env):
    ...
    if length == 0:
        ...
        return scheme_eval(expressions[0], env)
    elif scheme_false(scheme_eval(expressions[0], env)):
        return False
...
def do_or_form(expressions, env):
    ...
    if length == 0:
        return False
    elif scheme_true(scheme_eval(expressions[0], env)):
        return scheme_eval(expressions[0], env)
...
def make_let_frame(bindings, env):
    ...
    formals += [b[0] for b in bindings if scheme_symbolp(b[0])]
    vals += [scheme_eval(b[1], env) for b in bindings]
...
def check_formals(formals):
    ...
    for symbol in formals:
        ...
        if symbol in current_formals:
            raise SchemeError("repeat symbol")
        else:
            current_formals.append(symbol)
...
def do_mu_form(expressions, env):
    ...
    if expressions.second.second == nil:
        ...
        return MuProcedure(formals, Pair("begin", expressions.second))
